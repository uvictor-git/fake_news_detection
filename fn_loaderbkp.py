...
class FnLoader:
    ...
    def __loadFn(self):
        ...
        fn_data_train  = pd.read_csv("./fn_data/train_data.csv", delimiter=',', header=None).values
        fn_data_test = pd.read_csv("./fn_data/test_data.csv", delimiter=',', header=None).values
        fn_data_unl = pd.read_csv("./fn_data/train_unlabeled.csv", delimiter=',', header=None).values
...
#        print (f_data)
 #       print ("hello")
	...
	print(test_X.shape)
	print(test_y.shape)
        np.append(train_X, fn_data_unl[:-1], axis=0)
        np.append(train_y, fn_data_unl[-1], axis=0)
    ...
    def download_images_and_generate_tf_record(self):
        ...
        labeled_train_y = []
        unlabeled_train_X = np.empty(shape=(0, 30*100*1))
        validation_X = np.empty(shape=(0, 30*100*1))
        validation_y = []
